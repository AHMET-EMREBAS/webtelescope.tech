...
const Paths = getApiPaths(Product.name);

@Controller({
  tags: [ProductController.name],
})
export class ProductController {
  constructor(
    protected readonly service: ProductService,
    @InjectRepository(ProductView)
    protected readonly viewService: Repository<ProductView>
  ) {}
  ...
  async save(@Body() body: CreateProductDto) {
    return await this.service.saveSafe(body);
  }

  @Update({ path: Paths.BY_ID_PATH })
  update(@SourceId() id: number, @Body() body: UpdateProductDto) {
