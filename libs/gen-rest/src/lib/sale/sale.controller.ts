...
import { SaleService, SaleViewService } from './sale.service';

const Paths = getApiPaths(Sale.name);

@Controller({
  tags: [SaleController.name],
})
export class SaleController {
  constructor(
    protected readonly service: SaleService,
    protected readonly viewService: SaleViewService
  ) {}
  ...
  async save(@Body() body: CreateSaleDto) {
    return await this.service.saveSafe(body);
  }

  @Update({ path: Paths.BY_ID_PATH })
  update(@SourceId() id: number, @Body() body: UpdateSaleDto) {
