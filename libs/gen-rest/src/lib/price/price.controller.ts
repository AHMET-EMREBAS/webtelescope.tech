...
const Paths = getApiPaths(Price.name);

@Controller({
  tags: [PriceController.name],
})
export class PriceController {
  constructor(
    protected readonly service: PriceService,
    @InjectRepository(PriceView)
    protected readonly viewService: Repository<PriceView>
  ) {}

  @Get({ path: Paths.PLURAL_PATH })
  async findAll(
    @Query() paginator: PaginatorDto,
    @Query() queryDto: QueryPriceDto
  ) {
  ...
  async save(@Body() body: CreatePriceDto) {
    return await this.service.saveSafe(body);
  }

  @Update({ path: Paths.BY_ID_PATH })
  update(@SourceId() id: number, @Body() body: UpdatePriceDto) {
